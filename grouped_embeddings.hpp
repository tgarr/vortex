...
class GroupedEmbeddings{
     ...
     std::unique_ptr<faiss::gpu::GpuIndexIVFFlat> gpu_ivf_index; // FAISS index object. Initialize if use GPU IVF search
...
public:
     ...
     ***/
      ...
      * @param I: index array to store the index of the top_k embeddings
     ***/
     int faiss_gpu_flatl2_search(int nq, float* xq, int top_k, float* D, long* I){
      ...
      * initalize it if use faiss_gpu_ivf_search()
     ***/
     void initialize_gpu_ivf_search(){
          int nlist = 100;
          this->gpu_res = std::make_unique<faiss::gpu::StandardGpuResources>();
          this->gpu_ivf_index = std::make_unique<faiss::gpu::GpuIndexIVFFlat>(this->gpu_res.get(), this->emb_dim, nlist, faiss::METRIC_L2);
          this->gpu_ivf_index->add(this->num_embs, this->embeddings); // add vectors to the index
     }
      ...
      * @param I: index array to store the index of the top_k embeddings
     ***/
     int faiss_gpu_ivf_search(int nq, float* xq, int top_k, float* D, long* I){
          std::cout << "FAISS GPU ivf Search in [GroupedEmbeddings] class" << std::endl;
          this->gpu_ivf_index->search(nq, xq, top_k, D, I);
          // print results
